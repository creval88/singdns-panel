...
mkdir -p "$APP_DIR" "$INSTALL_DIR/updates" "$INSTALL_DIR/data"
...
chown -R "$RUN_USER:$RUN_USER" "$APP_DIR" "$INSTALL_DIR/updates" "$INSTALL_DIR/data" "$BIN_PATH"
chmod 775 "$INSTALL_DIR/updates" 2>/dev/null || true
chmod 775 "$INSTALL_DIR/data" 2>/dev/null || true
